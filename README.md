...
Note: Master branch of this library requires Model RealTime 12.0.2 release or later while release tag POCO requires Model RealTime 10.3 2018.48 or later, and further documentation can be found [here](https://model-realtime.hcldoc.com/help/topic/com.ibm.xtools.rsarte.webdoc/Articles/Integrations/TCPServer%20Library.html).
...
The original implementation of the library uses the POCO library and requires Model RealTime 10.3 2018.48 or later. Checkout the tag "POCO" if you want to use this version.
The latest implementation, that is available on the master branch, has no dependency on POCO, but instead requires Model RealTime 12.0.2 or later.
...
The maximum number of supported clients. This configuration is not available on POCO based release while available in master branch. This configuration requires Model RealTime release 12.0.2 or later.
- **port**:integer
